...
void CMainView::RefreshGraphs()
{
  ...
  if( m_ctlCOM.GetPortOpen() == TRUE)
	  m_ctlComButton.SetValue( true);
  else
...
void CMainView::OnTimer(UINT nIDEvent) 
{
	...
	if( nIDEvent == MY_TIMER_COM_FLUSH) {
        ...
        if( app->fhNew == NULL) {
          ...
          app->m_pLogger->LogTrace( "fhNew=%x", app->fhNew);
        }
				m_nPointsSkipped++;
...
{
  theApp.m_pLogger->LogTrace( "CMainView::OnValueChangedCwStart(): in");
  ...
  if( Value == TRUE)
    ...
    theApp.m_pLogger->LogTrace( "CMainView::OnValueChangedCwStart(): turning off... 1");

		m_ctlCOM.SetPortOpen( false);

    theApp.m_pLogger->LogTrace( "CMainView::OnValueChangedCwStart(): turning off... 2");

		KillTimer( MY_TIMER_COM_FLUSH);
		KillTimer( MY_TIMER_100);
		KillTimer( MY_TIMER_1000);
		
    theApp.m_pLogger->LogTrace( "CMainView::OnValueChangedCwStart(): turning off... 3");
    ...
    theApp.m_pLogger->LogTrace( "CMainView::OnValueChangedCwStart(): turning off... fh=%x", app->fhNew);
		/*if( app->fhNew != NULL)
			fclose( app->fhNew);*/
		app->fhNew = NULL;
    theApp.m_pLogger->LogTrace( "CMainView::OnValueChangedCwStart(): turning off... 5");

		gl_nCircleBufferGet = 0;
		gl_nCircleBufferPut = 0;

    theApp.m_pLogger->LogTrace( "CMainView::OnValueChangedCwStart(): turning off... 6");
    ...
    theApp.m_pLogger->LogTrace( "CMainView::OnValueChangedCwStart(): turning off... 7");
	}
...
void CMainView::OnOnCommComm() 
{
	...
	if( event > 1000) {
		if( event == 1001) return;
		if( event == 1004) return;
		if( event == 1006) return;
