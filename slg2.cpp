...
DWORD WINAPI BigThread(LPVOID lparam)
{
	...
	while( !m_bStopBigThreadFlag) {
      ...
      ptr = ( char *) &n_dN;
			...
			if( ( byte5 & 0x7f) == 16) {
				/*( ( CSlg2App *) AfxGetApp())->m_strSoftwareVer.Format(_T("v%d.%d.%d.%d"), 
							(byte1 / 16),		//major
							byte1 % 16,			//middle
							byte2 / 16,			//minor
							byte2 % 16);		//BIG_SMALL (2_0)
				 */
      ...
      if( btCheckSumm != byte12) {
        ...
        if( fabs( dSaTime) > gl_dec_coeff_low_filter) {
            ...
            */
				...
				switch( (byte5 & 0x7f)) {
					case 0: gl_avT3.AddPoint( dCur1);			break;
					case 1: gl_avT1.AddPoint( dCur1);			break;
          case 2: gl_avT2.AddPoint( dCur1);			break;
...
BOOL CSlg2App::InitInstance()
{
	...
	m_nLogLevel = GetProfileInt( _T("SETTINGS"), _T("LOGLEVEL"), 0);
